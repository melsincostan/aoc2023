...
fn compare_lines(a: &str, b: &str) -> u32 {
    let mut diff = 0;
    for i in 0..a.len() {
...
fn find_horizontal_symmetry(pattern: &str) -> Option<u32> {
    ...
    for i in 0..lines.len() - 1 {
        if compare_lines(lines[i], lines[i+1]) <= 1 {
            to_check.push(i);
        }
    }
    ...
    for pos in to_check {
        ...
        for i in 0..=pos {
            ...
            } else if compare_lines(lines[pos-i], lines[pos+i+1]) > 1 {
                valid = false;
                break;
            }
        }
        if valid && smudge_budget == 0 {
...
pub mod test {
    ...
    use crate::part2::{
        find_horizontal_symmetry, find_vertical_symmetry, solve, compare_lines,
    };
    ...
    fn test_find_horizontal_symmetry() {
        ...
        assert_eq!(find_horizontal_symmetry(&pattern_hor), Some(0));
        assert_eq!(find_horizontal_symmetry(&pattern_vert), Some(2));
    }
    ...
    fn test_find_vertical_symmetry() {
        let pattern_vert = fs::read_to_string("part_sample_vert.txt").unwrap();
        let pattern_hor = fs::read_to_string("part_sample_hor.txt").unwrap();
        assert_eq!(find_vertical_symmetry(&pattern_vert), None);
        assert_eq!(find_vertical_symmetry(&pattern_hor), None);
    }

    #[test]
    fn test_compare_lines() {
        assert_eq!(compare_lines("..##..##.", "..#.##.#."), 4);
        assert_eq!(compare_lines("#.##..##.", "..##..##."), 1);
    }

    #[test]
    fn test_solve() {
        assert_eq!(solve("sample.txt"), 400);
    }
}
